...
const generateMockArtists = (continentName: string) => {
  return Array.from({ length: 10 }).map((_, i) => ({
    name: `Artist ${i + 1}`,
    count: Math.floor(Math.random() * 1000) + 100,
    photo: `https://picsum.photos/seed/${continentName}-artist-${i}/150/150`,
    artwork: `https://picsum.photos/seed/${continentName}-art-${i}/300/200`,
    bio: `This is a brief introduction for Artist ${i + 1} from ${continentName}. They are known for their unique style and profound impact on the global art scene. Their works often explore themes of nature, society, and cultural heritage.`
...
export default function ContinentDetails() {
  ...
  const mockArtists = useMemo(() => generateMockArtists(decodedContinentName), [decodedContinentName]);
  ...
  return (
    <div className="relative h-screen w-full bg-[#EAE5D9] text-[#3A352D] font-sans overflow-hidden flex flex-col">
      ...
      <div className="flex-1 flex w-full h-full pt-24 pb-6 px-6 gap-6 z-10">
        ...
        {/* Left Column: Static Stats (2x2 Grid) */}
        <div className="w-[30%] grid grid-cols-2 grid-rows-2 gap-4 h-full">
          {/* Artwork Count */}
          <div className="bg-[#F4EFE6]/70 backdrop-blur-md border border-[#D3CDBF]/50 rounded-xl p-4 shadow-[0_8px_32px_rgba(0,0,0,0.05)] flex flex-col">
            <h3 className="font-serif font-bold text-sm text-[#3A352D] border-b border-[#D3CDBF]/50 pb-2 mb-3">Artwork Count</h3>
            <div className="flex-1 rounded-lg overflow-hidden relative bg-[#D3CDBF]/30 flex items-center justify-center">
              <img 
                src={`https://picsum.photos/seed/${decodedContinentName}-stats1/400/300`} 
                alt="Artwork Stats" 
          ...
          {/* Artist Count */}
          <div className="bg-[#F4EFE6]/70 backdrop-blur-md border border-[#D3CDBF]/50 rounded-xl p-4 shadow-[0_8px_32px_rgba(0,0,0,0.05)] flex flex-col">
            <h3 className="font-serif font-bold text-sm text-[#3A352D] border-b border-[#D3CDBF]/50 pb-2 mb-3">Artist Count</h3>
            <div className="flex-1 rounded-lg overflow-hidden relative bg-[#D3CDBF]/30 flex items-center justify-center">
              <img 
                src={`https://picsum.photos/seed/${decodedContinentName}-stats2/400/300`} 
                alt="Artist Stats" 
                className="absolute inset-0 w-full h-full object-cover"
                referrerPolicy="no-referrer"
              />
          ...
          {/* Artwork Proportion */}
          <div className="bg-[#F4EFE6]/70 backdrop-blur-md border border-[#D3CDBF]/50 rounded-xl p-4 shadow-[0_8px_32px_rgba(0,0,0,0.05)] flex flex-col">
            <h3 className="font-serif font-bold text-sm text-[#3A352D] border-b border-[#D3CDBF]/50 pb-2 mb-3">Artwork Proportion</h3>
            <div className="flex-1 rounded-lg overflow-hidden relative bg-[#D3CDBF]/30 flex items-center justify-center">
              <img 
                src={`https://picsum.photos/seed/${decodedContinentName}-stats3/400/300`} 
                alt="Artwork Proportion" 
                className="absolute inset-0 w-full h-full object-cover"
                referrerPolicy="no-referrer"
              />
            </div>
          ...
          {/* Artist Proportion */}
          <div className="bg-[#F4EFE6]/70 backdrop-blur-md border border-[#D3CDBF]/50 rounded-xl p-4 shadow-[0_8px_32px_rgba(0,0,0,0.05)] flex flex-col">
            <h3 className="font-serif font-bold text-sm text-[#3A352D] border-b border-[#D3CDBF]/50 pb-2 mb-3">Artist Proportion</h3>
            <div className="flex-1 rounded-lg overflow-hidden relative bg-[#D3CDBF]/30 flex items-center justify-center">
              <img 
                src={`https://picsum.photos/seed/${decodedContinentName}-stats4/400/300`} 
                alt="Artist Proportion" 
                className="absolute inset-0 w-full h-full object-cover"
                referrerPolicy="no-referrer"
              />
            </div>
        ...
        <div className="flex-1 relative rounded-xl overflow-hidden border border-[#D3CDBF]/30 bg-[#EAE5D9] shadow-inner p-2">
          {selectedArtist ? (
            ...
            <div className="w-full h-full rounded-lg overflow-hidden relative animate-in fade-in duration-300">
              <img 
                src={`https://picsum.photos/seed/${decodedContinentName}-continent-landscape/800/800`} 
                alt={`${decodedContinentName} landscape`}
                className="absolute inset-0 w-full h-full object-cover"
                referrerPolicy="no-referrer"
              />
              <div className="absolute inset-0 bg-gradient-to-t from-[#3A352D]/80 via-transparent to-transparent flex items-end justify-center pb-10">
                <h2 className="text-5xl font-serif font-bold text-[#F4EFE6] drop-shadow-lg tracking-wide">{decodedContinentName}</h2>
              </div>
            </div>
          )}
        </div>
