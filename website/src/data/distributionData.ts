...
const MIN_DISTRIBUTION_DECADE = 1850;
...
const COUNTRY_DISPLAY_NAME_TO_NATIONALITY = new Map<string, string>(
  Object.entries(COUNTRY_NAME_TO_NATIONALITY).map(([country, nationality]) => [
    normalizeRegionKey(country),
    nationality,
  ]),
);

COUNTRY_DISPLAY_NAME_TO_NATIONALITY.set('usa', 'American');
COUNTRY_DISPLAY_NAME_TO_NATIONALITY.set('uk', 'British');

const NATIONALITY_TO_COUNTRY_NAME = new Map<string, string>();

for (const [country, nationality] of Object.entries(COUNTRY_NAME_TO_NATIONALITY)) {
  if (!NATIONALITY_TO_COUNTRY_NAME.has(nationality)) {
    NATIONALITY_TO_COUNTRY_NAME.set(nationality, country);
  }
}

NATIONALITY_TO_COUNTRY_NAME.set('American', 'USA');
...
function bucketMatchesDecade(label: string, decade: number | null) {
  const start = decadeStart(label);
  if (start === null || start < MIN_DISTRIBUTION_DECADE) return false;
  return decade === null || start === decade;
}
...
export function getAvailableDecades(mode: DistributionMode) {
  ...
  return buckets
    ...
    .filter((decade): decade is number => decade !== null && decade >= MIN_DISTRIBUTION_DECADE);
}
...
export function getCountryDisplayName(region: string) {
  return NATIONALITY_TO_COUNTRY_NAME.get(region) ?? region;
}

export function getCountryDistributionRegion(countryName: string) {
  const normalizedCountryName = normalizeRegionKey(countryName);
  return COUNTRY_DISPLAY_NAME_TO_NATIONALITY.get(normalizedCountryName) ?? countryName;
}
...
function getArtworkCategoryTotal(category: string) {
  return artworkBuckets('country')
    .filter(bucket => bucketMatchesDecade(bucket.decade, null))
    .reduce((sum, bucket) => (
